...
func sanitizeLikePattern(input string) (string, error) {
	...
	if count > 0 {
		...
		if len(stripped) < 1 {
			return "", errors.New("使用模糊搜索时，关键词长度至少为 1 个字符")
		}
		return input, nil
	}
...
func normalizeTokenSearchPattern(input string) string {
	if strings.HasPrefix(input, "sk-") {
		return strings.TrimPrefix(input, "sk-")
	}
	if strings.HasPrefix(input, "%sk-") {
		return "%" + strings.TrimPrefix(input, "%sk-")
	}
...
func buildTokenSearchPattern(input string, fuzzyByDefault bool) (string, error) {
	pattern, err := sanitizeLikePattern(input)
	if err != nil {
		return "", err
	}
	if fuzzyByDefault && !strings.Contains(pattern, "%") {
		return "%" + pattern + "%", nil
	}
	return pattern, nil
}
...
func SearchUserTokens(userId int, keyword string, token string, offset int, limit int) (tokens []*Token, total int64, err error) {
	...
	if keyword != "" || token != "" {
		...
		if hasFuzzy && int(count) > maxTokens {
			return nil, 0, errors.New("令牌数量超过上限，仅允许精确搜索，请勿使用 % 通配符")
		}
		fuzzyByDefault = !hasFuzzy && int(count) <= maxTokens
	}
	...
	if keyword != "" {
		keywordPattern, err := buildTokenSearchPattern(keyword, fuzzyByDefault)
		if err != nil {
			return nil, 0, err
		}
		if token == "" {
			tokenPattern, err := buildTokenSearchPattern(
				normalizeTokenSearchPattern(keyword),
				fuzzyByDefault,
			)
			if err != nil {
				return nil, 0, err
	...
	if token != "" {
		tokenPattern, err := buildTokenSearchPattern(
			normalizeTokenSearchPattern(token),
			fuzzyByDefault,
		)
		if err != nil {
			return nil, 0, err
...
func (token *Token) InsertWithRouteOverrides(overrides []ApiKeyRouteOverrideInput) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Create(token).Error; err != nil {
			return err
		}
		return replaceTokenRouteOverridesTx(tx, token.Id, overrides)
	})
}
...
func GetExistingUserTokenNames(userId int, names []string) ([]string, error) {
	...
	for start := 0; start < len(uniqueNames); start += chunkSize {
		...
		if err := DB.Model(&Token{}).
			...
			Pluck("name", &chunkNames).Error; err != nil {
			return nil, err
		}
		existingNames = append(existingNames, chunkNames...)
	}
...
func (token *Token) UpdateWithRouteOverrides(overrides []ApiKeyRouteOverrideInput) (err error) {
	defer func() {
		if shouldUpdateRedis(true, err) {
			gopool.Go(func() {
				err := cacheSetToken(*token)
...
func DeleteTokenById(id int, userId int) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("token_id = ?", id).Delete(&ApiKeyRouteOverride{}).Error; err != nil {
			return err
		}
...
func BatchDeleteTokens(ids []int, userId int) (int, error) {
	...
	if err := tx.Where("token_id IN (?)", ids).Delete(&ApiKeyRouteOverride{}).Error; err != nil {
		tx.Rollback()
		return 0, err
	}
