...
func formatUserLogs(logs []*Log, startIdx int) {
	for i := range logs {
		logs[i].ChannelName = ""
		if logs[i].Type == LogTypeConsume || logs[i].Type == LogTypeError {
			logs[i].Content = ""
		}
...
type TokenUsageStatQuery struct {
	LogType           int
...
func GetTokenUsageStats(query TokenUsageStatQuery) ([]TokenUsageStat, error) {
	tx := LOG_DB.Table("logs").
		Select("token_id, MAX(token_name) token_name, MAX(user_id) user_id, MAX(username) username, MAX("+logGroupCol+") group_name, COUNT(*) requests, COALESCE(SUM(quota), 0) quota, COALESCE(SUM(prompt_tokens), 0) prompt_tokens, COALESCE(SUM(completion_tokens), 0) completion_tokens, MIN(created_at) first_used_at, MAX(created_at) last_used_at").
		Where("token_id > ?", 0)
	if query.UserId > 0 {
		tx = tx.Where("user_id = ?", query.UserId)
	}
	if query.LogType != LogTypeUnknown {
		tx = tx.Where("type = ?", query.LogType)
	}
	...
	if query.UpstreamRequestId != "" {
		tx = tx.Where("upstream_request_id = ?", query.UpstreamRequestId)
	}

	var aggregates []tokenUsageAggregate
	if err := tx.Group("token_id").Order("quota DESC").Scan(&aggregates).Error; err != nil {
		return nil, err
	}
	if len(aggregates) == 0 {
	...
	tokenQuery := DB.Model(&Token{}).Select("id", "name", "user_id", commonGroupCol).Where("id IN ?", tokenIds)
	if query.UserId > 0 {
		tokenQuery = tokenQuery.Where("user_id = ?", query.UserId)
	}
	var tokens []Token
	...
	usernameMap := make(map[int]string, userIds.Len())
	if userIds.Len() > 0 {
		var users []struct {
			Id       int
			Username string
	...
	stats := make([]TokenUsageStat, 0, len(aggregates))
	for _, aggregate := range aggregates {
		token, ok := tokenMap[aggregate.TokenId]
		tokenName := aggregate.TokenName
		username := aggregate.Username
		group := aggregate.GroupName
		if ok {
			tokenName = token.Name
			if tokenUsername := usernameMap[token.UserId]; tokenUsername != "" {
		...
		stats = append(stats, TokenUsageStat{
			...
			FirstUsedAt:      aggregate.FirstUsedAt,
