...
function granularityForRangePreset(preset: TimeRangePreset): TimeGranularity {
  if (preset === 'last_24_hours' || preset === 'today' || preset === 'yesterday')
    return 'hour'
  if (preset === 'this_month' || preset === 'last_month') return 'week'
  return 'day'
}

// Highlights the matching quick-range button when the applied range matches a
...
function detectQuickRangePreset(
  ...
  for (const preset of TIME_RANGE_PRESETS) {
    const range = getPresetDateRange(preset.value)
    const startMatches =
      Math.abs(start.getTime() - range.start.getTime()) < 60_000
    const endMatches = Math.abs(end.getTime() - range.end.getTime()) < 60_000
    if (startMatches && endMatches) return preset.value
  }
  return null
}
...
export function ModelsFilter(props: ModelsFilterProps) {
  ...
  const [selectedRange, setSelectedRange] = useState<TimeRangePreset | null>(() =>
    detectQuickRangePreset(props.currentFilters)
  )
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (nextOpen) {
      const applied =
        props.currentFilters ?? buildDefaultDashboardFilters(props.preferences)
      setFilters(applied)
      setSelectedRange(detectQuickRangePreset(applied))
    }
    setOpen(nextOpen)
  }
  ...
  const handleReset = () => {
    const preset = props.preferences.defaultTimeRangePreset
  ...
  const handleQuickRange = (preset: TimeRangePreset) => {
    ...
    setFilters((prev) => ({
      ...
      start_timestamp: start,
      end_timestamp: end,
      time_granularity: granularityForRangePreset(preset),
    }))
    setSelectedRange(preset)
  }
