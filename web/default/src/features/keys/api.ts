...
export async function getApiKeyRouteOptions(): Promise<ApiKeyRouteOptionsResponse> {
  const res = await api.get('/api/token/route-options')
  return res.data
}
...
// Batch create API keys from CSV upload
export async function batchCreateApiKeysFromCsv(
  file: File
): Promise<ApiResponse<{ count: number; duplicate_names?: string[] }>> {
