...
function getEffectiveRouteBillingLabel(
  item: ApiKeyEffectiveRouteLine,
  t: ReturnType<typeof useTranslation>['t']
) {
  const routeLine = item.route_line
  if (!routeLine) return t('No route line')
  if (routeLine.billing_mode === 'per_request') {
    return t('Per-request model prices')
  }
  if (routeLine.billing_mode === 'expression') {
    return t('Custom model prices')
  }
  return t('Ratio {{ratio}}', {
    ratio: routeLine.default_ratio ?? 1,
  })
}
...
export function RouteStrategyCell({ apiKey }: { apiKey: ApiKey }) {
  const { t } = useTranslation()
  const routeItems = apiKey.effective_route_lines ?? []
  const customCount =
    apiKey.route_overrides_count ||
    apiKey.route_overrides?.length ||
    routeItems.filter((item) => item.is_custom).length
  ...
  const routeStrategyLabel =
    !isCustom || !firstCustomRouteName
      ...
      : customRouteItems.length > 1
        ? `${firstCustomRouteName} +${customRouteItems.length - 1}`
        : firstCustomRouteName

  return (
    <Tooltip>
      <TooltipTrigger render={<BadgeCell />}>
        <StatusBadge
          label={routeStrategyLabel}
          variant={isCustom ? 'info' : 'neutral'}
          copyable={false}
        />
      </TooltipTrigger>
      <TooltipContent side='top' className='w-[min(92vw,26rem)] max-w-none'>
        <div className='space-y-2 text-xs'>
          <div className='font-medium'>{t('Effective route')}</div>
          <div className='space-y-1.5'>
            {routeItems.length === 0 && (
              <div className='text-muted-foreground'>{t('No route line')}</div>
            )}
            {routeItems.map((item) => (
              <div
              ...
              >
                ...
                <span className='min-w-0'>
                  <span className='block truncate font-medium'>
                    {item.route_line?.name ?? t('No route line')}
                  </span>
                  <span className='text-muted-foreground block truncate'>
                    {getEffectiveRouteBillingLabel(item, t)}
                  </span>
                </span>
              </div>
            ))}
