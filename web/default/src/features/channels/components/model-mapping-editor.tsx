...
export function ModelMappingEditor(props: ModelMappingEditorProps) {
  ...
  const lastLocalValueRef = useRef(props.value)
  const hasParsedInitialValueRef = useRef(false)
  ...
  useEffect(() => {
    ...
    if (!hasParsedInitialValueRef.current) {
      hasParsedInitialValueRef.current = true
      lastLocalValueRef.current = props.value
      parseJsonToRows(props.value)
      return
    }
    if (props.value === lastLocalValueRef.current) {
      return
    }
    lastLocalValueRef.current = props.value
    parseJsonToRows(props.value)
  ...
  const syncRows = (updatedRows: MappingRow[]) => {
    ...
    if (duplicates.length > 0) {
      ...
      emitChange(DUPLICATE_MAPPING_SENTINEL)
      return
    }
