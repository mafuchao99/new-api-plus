...
/**
 * Calculate dashboard preset ranges. Current periods end at now, while past
 * periods cover their full calendar boundary.
 */
export function getPresetDateRange(
  ...
  if (preset === 'last_24_hours') {
    return getRollingDateRange(1, now)
  }

  if (preset === 'today') {
    return { start: getStartOfDay(now), end: now }
  }
  ...
  if (preset === 'this_week') {
    return { start: getStartOfWeek(now), end: now }
  }
