...
func TestSearchTokensFindsByNameOrKeyWithKeyword(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "name fuzzy pattern",
			target:   "/api/token/search?keyword=%25name-match%25&p=1&size=10",
			expected: nameToken.Id,
		},
		{
			name:     "key fuzzy pattern",
			target:   "/api/token/search?keyword=%25unique1234%25&p=1&size=10",
			expected: keyToken.Id,
		},
		{
			name:     "key with sk prefix fuzzy pattern",
			target:   "/api/token/search?keyword=%25sk-unique1234%25&p=1&size=10",
			expected: keyToken.Id,
		},
		{
			name:     "name plain keyword",
			target:   "/api/token/search?keyword=name-match&p=1&size=10",
			expected: nameToken.Id,
		},
		{
			name:     "key plain keyword",
			target:   "/api/token/search?keyword=unique1234&p=1&size=10",
			expected: keyToken.Id,
		},
		{
			name:     "single character plain keyword",
			target:   "/api/token/search?keyword=m&p=1&size=10",
			expected: nameToken.Id,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx, recorder := newAuthenticatedContext(t, http.MethodGet, tt.target, nil, 1)
			SearchTokens(ctx)
			...
			if page.Items[0].ID != tt.expected {
				t.Fatalf("expected token id %d, got %d", tt.expected, page.Items[0].ID)
			}
		})
	}
}
