...
func CovertOpenAI2Gemini(c *gin.Context, textRequest dto.GeneralOpenAIRequest, info *relaycommon.RelayInfo) (*dto.GeminiChatRequest, error) {
	...
	pendingToolCalls := make([]pendingToolCall, 0)
	...
	for _, message := range textRequest.Messages {
		if message.Role == "system" || message.Role == "developer" {
			...
			if name == "" && toolCallID != "" {
				if val, exists := toolCallNames[toolCallID]; exists {
					name = val
				}
			}
			if name == "" && len(pendingToolCalls) > 0 {
				name = pendingToolCalls[0].name
			}
			if name == "" {
				return nil, fmt.Errorf("gemini function response name is empty: tool message is missing name and no matching previous assistant tool_calls entry was found for tool_call_id %q", message.ToolCallId)
			}
			if len(pendingToolCalls) > 0 {
				removed := false
				if toolCallID != "" {
					for i, pending := range pendingToolCalls {
						if pending.id == toolCallID {
		...
		if message.ToolCalls != nil {
			...
			for _, call := range message.ParseToolCalls() {
				...
				if functionName != "" {
					...
					if toolCallID != "" {
						toolCallNames[toolCallID] = functionName
					}
				}
			}
		}
