...
func TestCovertOpenAI2GeminiToolResponseNameFromToolCallID(t *testing.T) {
	t.Parallel()
	...
	geminiReq, err := CovertOpenAI2Gemini(c, req, info)
	require.NoError(t, err)
	...
	require.Equal(t, "web_search", geminiReq.Contents[1].Parts[0].FunctionResponse.Name)
}

func TestCovertOpenAI2GeminiRejectsUnnamedToolResponse(t *testing.T) {
	t.Parallel()
	...
	geminiReq, err := CovertOpenAI2Gemini(c, req, info)
	require.Nil(t, geminiReq)
	require.Error(t, err)
	require.Contains(t, err.Error(), "function response name is empty")
	require.Contains(t, err.Error(), "missing_call")
}
